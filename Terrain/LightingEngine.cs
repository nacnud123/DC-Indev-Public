...
public class LightingEngine
{
    ...
    /// <summary>
    /// One-time full-world sky light BFS: seeds every voxel with sky light greater than 1 across every
    /// loaded chunk into the propagation queue, then drains the queue to completion (no per-tick cap).
    /// Intended to be run once after initial world generation to let sunlight spread sideways
    /// (e.g. under overhangs, into caves near the surface) beyond the straight-down column pass done
    /// in <see cref="CalculateInitialLighting"/>.
    /// </summary>
    ...
    /// <summary>
    /// One-time full-world block light BFS: drains whatever is currently queued (typically the
    /// emitters seeded by <see cref="CalculateInitialLighting"/> across all chunks) to completion.
    /// </summary>
