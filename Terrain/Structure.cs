...
/// (optionally randomizing certain block types on placement), picks placement locations for
/// surface and underground structures, and exports an arbitrary in-world block selection back out
/// to a new JSON structure file. JSON shape: <c>{ "name": string, "size": {"x","y","z"}, "blocks":
...
public class StructureLoader
{
    ...
    // Water level of the world; used as a sanity check to avoid placing surface/underground
    // structures below the ocean (which would bury or flood them).
    private const int SEA_LEVEL = 64;

    /// <summary>
    /// Picks a random (x, z) within the world bounds (up to 50 attempts), finds the terrain height
    /// there via <see cref="World.FindSpawnPosition"/>, and rejects sites that are below sea level or
    /// whose ground block is water/air (i.e. not solid ground). On the first valid site found, places
    /// the structure so its base sits one block into the ground (groundY - 1), offset by
    /// <paramref name="offset"/> to align the structure's local origin under that point. Used for
    /// surface decoration (e.g. villages, trees) during world generation.
    /// </summary>
    ...
    /// <summary>
    /// Picks a random (x, z) within the world bounds (up to 50 attempts) that has surface ground at
    /// or above sea level (used only as a validity check on the column, not the placement depth),
    /// then places the structure at a random Y within [minY, topY] where topY is clamped so the
    /// structure's full height fits below <paramref name="maxY"/>. Used for underground features
    /// (e.g. dungeons, ore veins as structures) during world generation.
    /// </summary>
