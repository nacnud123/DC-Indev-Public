...
public partial class World
{
    ...
    // 2D grid of chunks, indexed [chunkX, chunkZ]. Chunk (0,0) covers world blocks
    // x in [0,16), z in [0,16); chunk (1,0) covers x in [16,32), etc.
    ...
    /// <summary>
    /// Creates (or loads) a world of worldSize x worldSize chunks. First tries to load each chunk
    /// from disk (Serialization.Load); any chunk that has no save data is procedurally generated
    /// via TerrainGen using the given seed. Loading is done as a first pass over the whole grid
    /// before generation so that generation code can safely look at neighboring chunks that were
    /// loaded from disk.
    /// </summary>
    public World(int worldSize, int seed, WorldGenSettings settings = default)
    {
        ...
        // Tracks which chunks were successfully loaded from a save file, so the generation pass
        // below knows to skip them.
    ...
    /// <summary>
    /// One-time setup pass run right after world construction: seeds initial per-chunk lighting,
    /// propagates sunlight/block light across the whole grid, then builds every chunk's render
    /// mesh up front (instead of relying on the incremental per-frame dirty-chunk queue, since at
    /// startup literally every chunk is "dirty"). Also frees the terrain generator's scratch data
    ...
    /// <summary>
    /// Per-frame world maintenance: advances lighting propagation by one step if there's pending
    /// work, then rebuilds up to MAX_CHUNK_REBUILDS_PER_FRAME dirty chunk meshes. Called once per
    /// render frame (not per game tick) from the main loop.
    /// </summary>
    public void Update()
    {
    ...
    // Converts a world block coordinate into (which chunk, position inside that chunk) and reads
    // the block there. Chunks are 16 blocks wide, and 16 is a power of two, so ">> 4" (divide by
    // 16) finds the chunk index and "& 15" (remainder) finds the position inside it - both faster
    // than using / and % directly, and they also work correctly for negative coordinates.
    ...
    // Note: unlike GetBlock (which uses >> 4 / & 15 bit tricks), these coordinate-mapping methods
    // use explicit division/modulo with a floor-toward-negative-infinity adjustment
    // ("(worldX + 1) / Chunk.WIDTH - 1" for negative worldX). Both approaches are equivalent for
    // power-of-two chunk sizes; this form is kept here for historical/readability reasons.
    ...
    // Block-emitted light (torches, lava, etc.), separate from sky light. Same out-of-bounds
    // handling as GetSkyLight but defaults to 0 (no light) rather than 15, since there's no
    // artificial light source outside the loaded world.
    ...
    // Raw block-light write, same caveat as SetSkyLightDirect - intended for the lighting engine's
    // own propagation, not general gameplay code.
    ...
    // Reads the 4-bit metadata nibble for the block at this world position (facing direction for
    // stairs/torches/etc. - see Chunk's class comment for the nibble-packing scheme). Returns 0
    // (the "default facing" value) for out-of-range positions.
    ...
    // Force every currently-loaded chunk to be treated as modified, so a full save writes them
    // all to disk regardless of whether they were actually touched (e.g. used for "save and
    // quit" or migrating an old save format).
    ...
    public void SetBlock(int worldX, int worldY, int worldZ, BlockType type)
    {
        ...
        // If this change happened right on a chunk boundary, the neighboring chunk's mesh also
        // depends on this block (see MarkChunkDirtyAt for the same pattern) - mark it dirty too.
        if (localX == 0 && chunkX > 0)
            mChunks[chunkX - 1, chunkZ].MarkDirty();
        if (localX == Chunk.WIDTH - 1 && chunkX < SizeInChunks - 1)
            mChunks[chunkX + 1, chunkZ].MarkDirty();
    ...
    // Shared culling/iteration logic for both the opaque and transparent render passes. Also
    // doubles as chunk load/unload bookkeeping: any chunk outside render distance gets
    // IsLoaded = false (see World.Ticks.cs / DoScheduledTick, which skips unloaded chunks).
    ...
    // Unculled variant: renders every chunk in the grid regardless of distance/frustum. Used by
    // things like the isometric screenshot renderer (IsoScreenshot) that need the whole world
    // visible rather than only what's near an in-game camera.
