...
/// <summary>
/// Produces a static top-down "map" style isometric snapshot of the world and writes it to disk as a PNG.
/// Bound to the F7 key in-game. Important: this class does NOT touch OpenGL or the GPU at all - it is a
/// pure CPU rasterizer that walks the <see cref="World"/> block data directly and paints pixels into a
/// byte buffer with ImageSharp. Because it only ever looks at the topmost solid block per column, caves,
/// overhangs, and anything underground can never appear in the output.
/// </summary>
public class IsoScreenshot
{
    ...
    // CPU-side copy of world.png's RGBA pixels, loaded once per Capture() call so SampleTopColor
    // can average tile colors without touching the GPU texture.
    ...
    public void Capture()
    {
        ...
        // 1px-tall diamond footprint, and each unit of height adds 2px of vertical offset. The canvas
        // must be wide/tall enough to hold every diagonal (X-Z) and vertical (Y) projection without clipping.
        int canvasW = (worldW + worldL) * 2;
        int canvasH = (worldW + worldL) + worldH * 2;

        var canvas = new byte[canvasW * canvasH * 4];

        // Reuses the same day/night brightness curve as the live sky: sin() of the time-of-day angle,
        // clamped so it never fully goes to black (0.05 floor) and normalized back to a 0..1 dayFactor.
        ...
        // Scan every world column (X,Z) independently - this is what makes the render "cave-free":
        // only the highest surface block per column is ever sampled, so anything beneath it is invisible.
        for (int worldX = 0; worldX < worldW; worldX++)
        {
            for (int worldZ = 0; worldZ < worldL; worldZ++)
            {
                ...
                // Dimetric projection: X-Z diagonal position maps to canvas X (world X minus Z, scaled
                // and re-centered by canvasW/2), while canvas Y combines the X+Z diagonal depth with a
                // height offset so taller columns are drawn further up the canvas.
    ...
    /// This top-down-only scan is the entire reason caves never show up in the screenshot.
    /// </summary>
    ...
    private (byte r, byte g, byte b) SampleTopColor(BlockType blockType)
    {
        ...
        // TopLeft.X/Y are normalized (0..1) UV coordinates; multiplying by ATLAS_TILES converts
        // them back into integer tile-grid column/row indices (see TextureCoords.cs / UvHelper).
    ...
    // Writes a single opaque RGBA pixel into the flat canvas buffer, bounds-checked via unsigned
    // comparison (a negative cx/cy wraps to a huge uint and fails the >= check, same as an overflow).
