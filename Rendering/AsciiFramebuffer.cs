...
/// <summary>
/// Off-screen render target used for the ASCII post-process effect: the normal scene is
/// rendered into this framebuffer's color texture (with a depth/stencil renderbuffer for
/// correct depth testing), and a later full-screen pass samples <see cref="ColorTexture"/>
...
public class AsciiFramebuffer : IDisposable
{
    ...
    // Combined depth+stencil renderbuffer attachment; not sampled directly, only used so depth testing
    // behaves correctly while rendering the scene into this offscreen target.
    private uint _rbo;
    // Framebuffer dimensions in pixels, matching the window/render resolution this was created for.
    private int _width, _height;

    /// <summary>
    /// Allocates the framebuffer, its color texture, and a depth/stencil renderbuffer at the
    /// given pixel dimensions. Must be recreated (not resized) if the render resolution changes.
    /// </summary>
    public AsciiFramebuffer(int width, int height)
    {
        ...
        unsafe
        {
            // Allocate storage only (null data pointer) - the texture is populated later by
            // rendering into the framebuffer, not by uploading pixel data here.
        ...
        // Nearest filtering keeps the source pixels crisp/blocky, which matters since the ASCII
        // pass samples fixed-size pixel blocks to choose a glyph - smoothing would blur that signal.
        gl.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMinFilter, (int)TextureMinFilter.Nearest);
        gl.TexParameter(TextureTarget.Texture2D, TextureParameterName.TextureMagFilter, (int)TextureMagFilter.Nearest);
        gl.FramebufferTexture2D(FramebufferTarget.Framebuffer, FramebufferAttachment.ColorAttachment0,
            TextureTarget.Texture2D, ColorTexture, 0);

        // Depth24Stencil8 renderbuffer: needed so normal depth-tested scene rendering works when
        // targeting this FBO, even though only color is read back afterward.
