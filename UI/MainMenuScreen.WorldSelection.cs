...
{
    /// <summary>
    /// Renders the saved-worlds list with Play/Delete/New World/Back actions. Lazily refreshes
    /// the world list from disk the first time this screen is shown (<see cref="mWorldsLoaded"/>
    /// guard) rather than every frame. Only called while <c>mCurrentState == MainMenuState.WorldSelection</c>.
    /// </summary>
    private void RenderWorldSelectionScreen(ImGuiWindowFlags flags)
    {
        ...
        // ImGui popups must be opened via OpenPopup() before BeginPopupModal() will show them;
        // mShowDeleteConfirm is a one-frame trigger flag set by the Delete button or Del key,
        // consumed here and reset immediately so it only fires the popup once.
    ...
    // Modal confirmation dialog for world deletion; must be called every frame (even when not
    // open) since ImGui's popup ID needs BeginPopupModal to be reached for OpenPopup to work.
