...
{
    /// <summary>
    /// Renders the title/landing sub-screen: game title, splash text, Start/Options/Quit
    /// buttons, and a reference list of current keybindings in the bottom-left corner.
    /// This is the default/first sub-screen shown when the menu opens (<see cref="MainMenuState.Title"/>).
    /// </summary>
    private void RenderTitleScreen(ImGuiWindowFlags windowFlags)
    {
        ...
        // Controls (bottom-left) - reads live keybindings via KeyName() so this list stays
        // accurate even after the player rebinds keys on the Keybindings screen.
        ImGui.PushStyleColor(ImGuiCol.Text, ColTextDim);
        string[] controls =
        {
        ...
        // Stack the control lines upward from the bottom edge, sized to fit however many
        // entries `controls` has (so adding/removing a line doesn't need a manual offset fix).
