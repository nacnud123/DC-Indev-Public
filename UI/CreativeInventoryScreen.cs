...
/// <summary>
/// Creative-mode item picker: a tabbed grid of every block/item flagged as selectable,
/// clicking one puts it directly in the player's hand/hotbar (no crafting or inventory
/// slots involved - creative mode has unlimited items). Uses ImGui image buttons rather
/// than the custom slot-drawing used by <see cref="InventoryScreenBase"/>.
/// </summary>
public class CreativeInventoryScreen
{
    ...
    // Row width is computed per-row (not just per-grid) so the last, possibly-partial
    // row is still centered on its own rather than left-aligned under a full grid.
    ...
    private void RenderItemButton(int index, ItemType type, Item item)
    {
        ...
        // Flip V: atlas coords are top-left origin but ImGui image UVs expect
        // bottom-left-origin sampling here, so swap TopLeft.Y/BottomRight.Y.
