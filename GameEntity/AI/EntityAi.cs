...
/// <summary>
/// Abstract base class for all mob AI controllers (passive and hostile).
/// Owns a shared A* pathfinder instance and provides common helper logic used
/// by subclasses: path recalculation, arrival checks, ground-level scanning,
/// step-up/jump detection, and body-facing. Subclasses implement <see cref="Tick"/>
/// to drive their own state machine (idle/wander/chase/attack/etc).
/// </summary>
public abstract class EntityAi
{
    ...
    // One pathfinder per AI instance; AStarPathfinder caches its working sets internally
    // between calls, so each mob gets its own to avoid cross-talk between concurrent searches.
    ...
    // Set by OnHurt() when the entity takes damage; consumed by subclasses to react
    // (e.g. immediately start chasing/aggro) on the next Tick.
    protected bool WasHurt;

    // Overridden by AI types that have a flee state (e.g. low-health passive mobs) so
    // other systems can query whether this mob is currently running away.
    ...
    /// <summary>
    /// Runs one game-tick's worth of AI logic. Called every tick from the entity's update loop.
    /// Implementations own their own state machine and are responsible for setting
    /// ParentEntity.Velocity/Yaw as needed.
    /// </summary>
    public abstract void Tick(World world);

    /// <summary>
    /// Recomputes <see cref="CurrentPath"/> from the entity's current (floored) position
    /// to <see cref="CurrentTarget"/> using A*. No-op if no target is set. Expensive relative
    /// to other AI work, so callers should gate this behind a cooldown timer rather than
    ...
    /// <summary>
    /// True once the entity is horizontally within 0.25 blocks of the center of
    /// CurrentTarget on both axes (Y is intentionally ignored - only XZ proximity matters
    /// for "have we arrived" purposes). Also true if there is no target at all.
    /// </summary>
    ...
    /// <summary>
    /// Scans vertically at the given (x, z) column for the nearest walkable Y level
    /// (solid block below, two air blocks above for feet+head clearance) to the entity's
    /// current height. Searches downward first from the entity's current Y, then a short
    /// distance upward (up to 4 blocks) if nothing was found below. Returns -1 if no
    /// walkable ground is found in range.
    /// </summary>
    protected int FindGroundLevel(World world, float x, float z)
    {
        ...
        // Upward: only check a small window (5 blocks) above the entity - covers stepping
        // onto nearby raised terrain without scanning the whole column.
    ...
    /// <summary>
    /// True if moving one block in the given horizontal direction (dirX, dirZ) would run the
    /// entity into a solid block at foot height with clear space above it (i.e. a single-block
    /// step the entity should jump over rather than walk into).
    /// </summary>
    ...
    // Rotates the entity's yaw to face its current horizontal velocity direction.
    // The -PI/2 offset accounts for the model's forward axis not being aligned with +Z.
    ...
    /// <summary>
    /// Called by the owning entity when it takes damage. Sets a flag consumed on the
    /// next Tick so subclasses can react (e.g. instantly aggro/chase the attacker).
    /// </summary>
    public void OnHurt()
    {
        WasHurt = true;
