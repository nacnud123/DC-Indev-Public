...
public abstract class EntityAi
{
    ...
    protected readonly AStarPathfinder Pathfinder = new();
    ...
    protected void RecalculatePath(World world)
    {
        ...
        CurrentPath = Pathfinder.FindPath(world, startPos, CurrentTarget.Value);
    }
    ...
    {
        int checkX = (int)MathF.Floor(ParentEntity.Position.X + dirX * 0.5f);
        int checkY = (int)MathF.Floor(ParentEntity.Position.Y);
        int checkZ = (int)MathF.Floor(ParentEntity.Position.Z + dirZ * 0.5f);
