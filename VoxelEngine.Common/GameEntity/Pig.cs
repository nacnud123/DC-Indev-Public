// Pig entity with rigid-body part animation (body, head, legs) | DA | 2/11/26
...
public class Pig : Entity
{
    ...
    private const string BODY_MODEL   = "Resources/Entities/Pig/PigBody/PigBody.obj";
    private const string BODY_TEXTURE = "Resources/Entities/Pig/PigBody/texture.png";
    private const string HEAD_MODEL   = "Resources/Entities/Pig/PigHead/PigHead.obj";
    private const string HEAD_TEXTURE = "Resources/Entities/Pig/PigHead/Head.png";
    private const string LEG_MODEL    = "Resources/Entities/Pig/PigLeg/PigLeg.obj";
    private const string LEG_TEXTURE  = "Resources/Entities/Pig/PigLeg/Leg.png";

    // Local-space offsets (in model units, pre-Scale) placing each part relative to the entity origin.
    private static readonly Vector3 BodyOffset         = new(-0.0625f, .062f, 0f);
    private static readonly Vector3 HeadOffset         = new(0.1875f, 0.2f, 0.03125f);
    ...
    private const float HEAD_LOOK_RANGE = 8f;               // blocks; player must be within this radius for the pig to look at them
    ...
    private readonly IRenderHandle? mBodyModel;
    private readonly IRenderHandle? mHeadModel;
    private readonly IRenderHandle? mLegModel; // single leg mesh reused for all four legs via DrawLeg
    ...
    /// <summary>Spawns a pig at the given world-space position, loads its part models, and attaches passive wander AI.</summary>
    public Pig(Vector3 position)
    {
        Position = position;
        InitShader();
        mBodyModel = RenderBackend.Current.LoadModel(BODY_MODEL, BODY_TEXTURE);
        mHeadModel = RenderBackend.Current.LoadModel(HEAD_MODEL, HEAD_TEXTURE);
        mLegModel  = RenderBackend.Current.LoadModel(LEG_MODEL, LEG_TEXTURE);
        CurrentAI  = new PassiveEntityAi(this);
        // Random 5-15s delay before the first idle oink so multiple pigs don't sync up.
        mIdleSoundTimer = 5f + (float)GameContext.Current.GameRandom.NextDouble() * 10f;
    }
    ...
    public override void Tick(World world)
    {
        ...
        {
            mIdleSoundTimer = 5f + (float)GameContext.Current.GameRandom.NextDouble() * 10f;
            int idx = GameContext.Current.GameRandom.Next(1, 4); // picks one of PigIdle1..3.ogg
            GameContext.Current.AudioManager.PlayAudio(
                $"Resources/Audio/Entities/Pig/PigIdle{idx}.ogg",
                // Proximity attenuates volume based on distance to the player, falling off to 0 at 20 blocks.
                Proximity(World.DistanceToNearestPlayer(Position) ,20f, GameContext.Current.AudioManager.SfxVol),
                false);
        }
    }

    /// <summary>Applies damage, plays the hurt/death sound, and on death drops 1-3 raw pork items into the world.</summary>
    public override void TakeDamage(int amount)
    {
        ...
        if (!IsAlive)
        {
            int count = GameContext.Current.GameRandom.Next(1, 4); // 1-3
            var drop = new DroppedItemEntity(Position, ItemStack.FromItem(ItemType.RawPork, count));
            GameContext.Current.GetWorld.AddEntity(drop);
        }
    }
    ...
    // As a client-side proxy it never ticks; its legs still have to move.
    protected override void TickProxyAnimation() => UpdateAnimation();
    ...
    private void UpdateAnimation()
    {
        ...
        {
            // Exponential ease-out back to a neutral stance once the pig stops moving.
        ...
        if (hSpeed < 0.01f)
        {
            var lookAt = World.NearestPlayerTo(Position);
            if (lookAt == null)
                return;                     // empty server: nothing to look at

            Vector3 toPlayer = lookAt.Position - Position;
            ...
            if (distSq < HEAD_LOOK_RANGE * HEAD_LOOK_RANGE && distSq > 0.01f)
            {
                ...
                float dy = (lookAt.Position.Y + 0.9f) - (Position.Y + HeadOffset.Y * Scale);
