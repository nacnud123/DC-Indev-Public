...
public class Stalker : Entity
{
    ...
    public override void Tick(World world)
    {
        ...
        var target = World.NearestPlayerTo(Position);
        if (target == null)
            return;
    ...
    private void UpdateFuse(World world, float distToPlayer, float dt)
    {
        if (!mFuseActive)
        {
            if (distToPlayer <= FUSE_RANGE)
            {
                ...
                int vol = Proximity(distToPlayer, 20f, GameContext.Current.AudioManager.SfxVol);
                GameContext.Current.AudioManager.PlayAudio("Resources/Audio/TNTHiss.ogg", vol);
            }
    ...
    protected override void DrawModel(Matrix4x4 view, Matrix4x4 projection)
    {
        ...
        if (mFuseActive)
            RenderBackend.Current.SetFloat("uHitFlash", mFlashOn ? 0.85f : 0f);
    ...
    public override void TakeDamage(int amount)
    {
        ...
        if (!IsAlive)
        {
            ...
            int count = GameContext.Current.GameRandom.Next(0, 1);
            if (count > 0)
            {
