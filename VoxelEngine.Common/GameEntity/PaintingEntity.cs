...
/// <summary>
/// A painting hung on a wall. Unlike most entities, it has no physics/gravity and never moves
/// once placed - it just sits at a fixed anchor block/facing and periodically checks that its
/// wall is still there. Rendering is intentionally a no-op here (<see cref="DrawModel"/>) because
/// paintings are drawn separately by a dedicated PaintingRenderer that batches painting art
/// textures, rather than going through the normal per-entity model draw path.
/// </summary>
public class PaintingEntity : Entity
{
    ...
    // Counts ticks since the last wall-validity check; only re-validates periodically rather than
    // every tick since checking every surrounding block is relatively expensive and the wall
    // rarely changes.
    ...
    // Direction along the wall surface that increases the painting's local "right" (X) axis,
    // indexed by Facing - used to walk across the painting's width when checking wall blocks.
    ...
    // Computes the painting's visual center in world space from its anchor block, facing, and
    // size. The anchor is treated as the bottom-left wall block the painting is nailed to.
    ...
    // Builds a thin AABB representing the painting's flat canvas, used both for entity targeting
    // (aiming at it to break it) and for overlap checks against other paintings. SizeX/SizeY are
    // in pixels (32px = 1 block), and a small inset (-0.01) avoids exact-edge intersections with
    // neighboring paintings/blocks causing false positives.
    ...
    // Checks that every wall block behind the painting is still solid and every block directly in
    // front of the painting is still air (nothing built into it), and that it doesn't overlap any
    // other painting. Used to detect when the supporting wall was mined out from under it.
    ...
    // Any damage (e.g. player punching it) immediately knocks the painting off the wall - there's
    // no health/durability, one hit is always enough.
    public override void TakeDamage(int amount) => Drop(GameContext.Current.GetWorld);

    public override Aabb GetBoundingBox() => GetPaintingAabb();

    // Removes the painting entity and spawns a pickup-able item in its place, slightly offset out
    // from the wall so it doesn't spawn inside the (now possibly missing) wall block.
    ...
    // Intentionally empty - paintings are rendered by a separate batched PaintingRenderer, not
    // through the normal per-entity draw path.
