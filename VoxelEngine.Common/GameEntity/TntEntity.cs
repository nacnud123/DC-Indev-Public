...
public class TntEntity : Entity
{
    ...
    private readonly IRenderHandle? mWorldAtlas; // opaque handle; the client owns the actual GL texture
    private IRenderHandle? mMesh;                // the cube's uploaded mesh, null on a headless host
    ...
    // Dropped the `Texture worldAtlas` parameter: requiring a GPU texture to prime a block of TNT
    // meant Block.Explode couldn't run without a renderer. The atlas comes from the backend now.
    public TntEntity(Vector3 position, Block parentBlock, float fuseDuration = 4f,
        int explosionRadius = 4, float explosionPower = 4.0f)
    {
        ...
        mWorldAtlas = RenderBackend.Current.WorldAtlas;
        BuildCubeMesh();
        GameContext.Current?.AudioManager.PlayAudio("Resources/Audio/TNTHiss.ogg", GameContext.Current.AudioManager.SfxVol);
    }
    ...
    protected override void DrawModel(Matrix4x4 view, Matrix4x4 projection)
    {
        ...
        if (mMesh != null)
            RenderBackend.Current.DrawMesh(mMesh, mWorldAtlas, mvp);
    }
