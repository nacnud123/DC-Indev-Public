...
public class Sheep : Entity
{
    ...
    private readonly IRenderHandle? mBodyModel;      // woolless body, shown once sheared
    private readonly IRenderHandle? mBodyWoolModel;  // fluffy wool overlay body, shown while unsheared
    ...
    public Sheep(Vector3 position)
    {
        ...
        mBodyWoolModel = RenderBackend.Current.LoadModel(BODY_WOOL_MODEL, BODY_WOOL_TEXTURE);
    ...
    public override void Tick(World world)
    {
        ...
        {
            mIdleSoundTimer = 5f + (float)GameContext.Current.GameRandom.NextDouble() * 10f;
            int idx = GameContext.Current.GameRandom.Next(1, 4); // picks one of SheepIdle1..3.ogg

            // Proximity attenuates volume based on distance to the player, falling off to 0 at 20 blocks.
            GameContext.Current.AudioManager.PlayAudio($"Resources/Audio/Entities/Sheep/SheepIdle{idx}.ogg", Proximity(World.DistanceToNearestPlayer(Position), 20f, GameContext.Current.AudioManager.SfxVol), false);
        }
    }
    ...
    public override void TakeDamage(int amount)
    {
        ...
        GameContext.Current.AudioManager.PlayAudio("Resources/Audio/Entities/Sheep/SheepDie.ogg", Proximity(World.DistanceToNearestPlayer(Position), 20f, GameContext.Current.AudioManager.SfxVol), false);
        ...
        {
            int count = GameContext.Current.GameRandom.Next(1, 4);
