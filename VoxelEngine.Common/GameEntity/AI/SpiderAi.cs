...
/// <summary>
/// Hostile AI for the Spider mob. Unlike a plain HostileEntityAi, spiders have an
/// additional aggro gate layered on top of the base chase/attack state machine:
/// they only engage the player while ambient brightness at their position is below
/// BRIGHT_THRESHOLD (Minecraft-style "spiders are neutral in daylight" behaviour), with a
/// small per-tick chance to deaggro once back in bright light. While not aggroed they run
/// their own lightweight passive wander instead of delegating to base.Tick. Also adds a
/// chance-based leap attack when within a mid-range band of the player.
/// </summary>
public class SpiderAi : HostileEntityAi
{
    ...
    // Brightness (0..1) threshold at/above which the spider is considered to be in "daylight"
    // for aggro purposes.
    ...
    // Leap only triggers within this band of distances (in blocks) - too close and a melee
    // swing suffices, too far and a leap wouldn't close the gap usefully.
    ...
    // True once the spider has aggroed the player; gates whether base.Tick (chase/attack)
    // or PassiveWander runs each tick.
    private bool mIsAggroed;

    // Cached wander heading/timer for the non-aggroed passive-wander behaviour (mirrors the
    // wander logic in HostileEntityAi.Wander, duplicated here since passive wandering runs
    // independently of the base class's own state machine while not aggroed).
    ...
    // Computes an effective "how lit is this spot" value in [0,1], accounting for the
    // day/night cycle's effect on skylight (skylight is dimmed at night via skylightSub)
    // as well as any nearby block light (torches, lava, etc). Chunk.MAX_LIGHT is the max
    // raw light level (15), used to normalize into [0,1].
    private float GetBrightness(World world)
    {
        ...
        // Sun angle derived from time-of-day (0..1 over the full day/night cycle); sin() gives
        // a smooth day/night brightness curve. *2 and clamp compress the curve so full daylight
        // brightness is reached well before solar noon rather than only exactly at noon.
        float sunAngle = GameContext.Current.TimeOfDay * MathF.PI * 2f;
        float sunlightLevel = Math.Clamp(MathF.Sin(sunAngle) * 2f, 0f, 1f);
        // At night (sunlightLevel=0) skylight is subdued by 15 (i.e. fully negated); at full
        // day (sunlightLevel=1) only subdued by 4, so some skylight attenuation from depth/
        // overhangs still applies. Interpolates between those two extremes.
        int skylightSub = (int)(15f + (4f - 15f) * sunlightLevel);

        int skyLight = world.GetSkyLight(x, y, z);
        int blockLight = world.GetBlockLight(x, y, z);
        // Effective light is the max of block light and (day-adjusted) skylight - matches how
        // actual rendering/mob-spawn light level is computed elsewhere in the engine.
    ...
    public override void Tick(World world)
    {
        ...
        // Small per-tick chance to give up aggro once standing in bright light (avoids an
        // abrupt "instantly passive the moment it's bright enough" feel).
        if (mIsAggroed && brightness >= BRIGHT_THRESHOLD && Random.NextDouble() < DEAGGRO_CHANCE)
            mIsAggroed = false;

        // Only re-evaluate aggro-on-sight while dim; a spider standing in daylight won't
        // spontaneously aggro even if the player walks within DETECTION_RANGE.
        if (!mIsAggroed && brightness < BRIGHT_THRESHOLD)
        {
            var seen = World.NearestPlayerTo(ParentEntity.Position);
            if (seen != null && (seen.Position - ParentEntity.Position).Length() <= DETECTION_RANGE)
                mIsAggroed = true;
        }
        ...
        {
            // Not aggroed: clear any stale hurt flag (base.Tick never ran to consume it) and
            // run independent passive-wander movement instead.
            WasHurt = false;
            PassiveWander(world);
    ...
    protected override void OnAttackEntity(World world, float dist)
    {
        ...
        // Checked before the melee cooldown so leaps aren't gated by ATTACK_COOLDOWN.
        if (dist >= LEAP_MIN_DIST && dist <= LEAP_MAX_DIST && Random.NextDouble() < LEAP_CHANCE && ParentEntity.IsOnGround)
        {
            var leapTarget = World.NearestPlayerTo(ParentEntity.Position);
            if (leapTarget == null) return;

            Vector3 toPlayer = leapTarget.Position - ParentEntity.Position;
        ...
        var player = World.NearestPlayerTo(ParentEntity.Position);
    ...
    // Random-walk wander used while not aggroed; mirrors HostileEntityAi.Wander (new heading
    // every 60-120 ticks, 40% chance to stand still) but runs independently of the base
    // class's own state machine since the spider bypasses base.Tick entirely while passive.
