...
public class PlayerInventory
{
    ...
    /// </summary>
    public bool TryAdd(ItemStack stack) => AddGetRemainder(stack) is not { } left || left.Count < stack.Count;

    /// <summary>
    /// Adds what fits and hands back what didn't, or null if the whole stack went in. This is the
    /// form callers want whenever the stack has somewhere else to go - dropping it on the floor,
    /// leaving it on the ground - because <see cref="TryAdd"/>'s bool cannot distinguish "all of it
    /// fit" from "one item fit and the other 63 have nowhere to go".
    /// </summary>
    public ItemStack? AddGetRemainder(ItemStack stack)
    {
        ...
        return remaining > 0 ? stack.WithCount(remaining) : null;
    }
