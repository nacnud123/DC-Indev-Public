...
public class ArrowEntity : Entity
{
    ...
    private static IRenderHandle? sMesh;
    private static int sVertexCount;
    private static bool sMeshReady;
    private static IRenderHandle? sTexture;
    ...
    private static void EnsureMesh()
    {
        ...
        sTexture = RenderBackend.Current.LoadTexture(TEXTURE_PATH);
        ...
        sMesh = RenderBackend.Current.CreateMesh(arr, sVertexCount);
    ...
    private void TickFlying(World world)
    {
        ...
        var player = World.NearestPlayerTo(Position);
        if (player != null && (player != mOwner || mTicksInAir >= OWNER_GRACE))
        {
        ...
        if (blockHit.Type == RaycastHitType.Block)
        {
            ...
            int vol = Proximity(dist, 20f, GameContext.Current.AudioManager.SfxVol);
            GameContext.Current.AudioManager.PlayAudio("Resources/Audio/Bow/ArrowHit.ogg", vol);
            return;
        }
    ...
    private void TryPickup()
    {
        ...
        // Local-inventory action, so local player only - the server owns pickup for everyone else.
    ...
    {
        // Built on first draw rather than in the constructor: firing an arrow is game logic that a
        // headless host runs too, and mesh upload needs a live GL context.
        EnsureMesh();

        if (!sMeshReady || sMesh == null)
            return;
        ...
        // doubleSided: the shaft's crossed-quad geometry needs both faces of each quad visible
        // from any angle (see EnsureMesh), so the backend drops backface culling for this draw.
        if (sMesh != null)
            RenderBackend.Current.DrawMesh(sMesh, sTexture, mvp, doubleSided: true);
    }
