...
/// <summary>
/// Places a painting entity on a wall. Unlike normal block placement, the painting's specific
/// artwork (size) is chosen randomly from whichever registered PaintingDef variants actually
/// fit the target wall surface, mirroring Minecraft's placement behavior.
/// </summary>
public class ItemPainting : Item
{
    ...
    /// <summary>
    /// Derives a cardinal facing from the direction placePos was offset from blockPos, then
    /// tries every registered painting size against that wall, keeping only ones that fit
    /// (IsValidSurface), and finally places a random one of the valid candidates.
    /// </summary>
    public override bool OnUse(World world, Vector3i blockPos, Vector3i? placePos)
    {
        ...
        var chosen = candidates[GameContext.Current.GameRandom.Next(candidates.Count)];
