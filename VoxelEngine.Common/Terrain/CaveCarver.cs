...
internal class CaveCarver
{
    ...
    public void GenerateCaves(World world, int chunkX, int chunkZ, int seed)
    {
        ...
        CarveInto(chunk, seed);
    }

    /// <summary>
    /// Carves a chunk that isn't in the world yet - the infinite generator's entry point, called
    /// from a generation worker.
    ///
    /// Safe off the main thread because the carver never reads or writes anything outside
    /// <paramref name="chunk"/>: cave seeds come from a per-column RNG derived purely from the world
    /// seed and column coordinates, so a tunnel starting eight chunks away is replayed identically
    /// by every chunk it crosses and each one keeps only the part inside its own bounds. That's
    /// what makes the caves line up across borders without neighbours having to exist.
    /// </summary>
    public static void CarveInto(Chunk chunk, int seed)
    {
        int minX = chunk.ChunkX * Chunk.WIDTH;
        int minZ = chunk.ChunkZ * Chunk.DEPTH;

        var carver = new CaveCarver(minX, minZ, minX + Chunk.WIDTH - 1, minZ + Chunk.DEPTH - 1)
        {
            mTargetChunk = chunk
        };

        carver.Run(chunk.ChunkX, chunk.ChunkZ, seed);
    }

    private void Run(int chunkX, int chunkZ, int seed)
    {
        for (int cx = chunkX - CAVE_SEARCH_RADIUS; cx <= chunkX + CAVE_SEARCH_RADIUS; cx++)
        {
            for (int cz = chunkZ - CAVE_SEARCH_RADIUS; cz <= chunkZ + CAVE_SEARCH_RADIUS; cz++)
            {
                ...
                if (rng.NextDouble() < WORM_CAVE_CHANCE)
                    SpawnWorms(cx, cz, rng, CarveWorm);

                if (rng.NextDouble() < BRANCHING_CAVE_CHANCE)
                    SpawnWorms(cx, cz, rng, (x, y, z, yaw, pitch, r, steps, rand) =>
                        CarveWormBranching(x, y, z, yaw, pitch, r, steps, rand, 0));

                if (rng.NextDouble() < CAVERN_CHANCE)
                    SpawnWorms(cx, cz, rng, CarveWormCavern);

                if (rng.NextDouble() < RAVINE_CHANCE)
                    CarveRavine(cx * Chunk.WIDTH + rng.Next(Chunk.WIDTH),
    ...
    private void SpawnWorms(int cx, int cz, Random rng, Action<float, float, float, float, float, float, int, Random> carver)
    {
        int count = 0;
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            carver(startX, startY, startZ, yaw, pitch, radius, length, rng);
        }
    }
    ...
    private void CarveWorm(float x, float y, float z, float yaw, float pitch, float radius, int steps, Random rng)
    {
    ...
    private void CarveWormBranching(float x, float y, float z, float yaw, float pitch, float radius, int steps, Random rng, int depth)
    {
        ...
        for (int step = 0; step < steps; step++)
        {
            ...
            if (step > 10 && rng.NextDouble() < 0.02)
            {
                ...
                CarveWormBranching(x, y, z, branchYaw, branchPitch, branchRad, steps / 2 + rng.Next(20), rng, depth + 1);
            }
    ...
    private void CarveWormCavern(float x, float y, float z, float yaw, float pitch, float radius, int steps, Random rng)
    {
        bool inCavern = false;
        ...
        for (int step = 0; step < steps; step++)
        {
            ...
            CarveSphere(x, y, z, effectiveRadius);
    ...
    private void CarveRavine(float startX, float startY, float startZ, Random rng)
    {
    ...
    private void CarveSphere(float cx, float cy, float cz, float radius)
    {
    ...
    private void CarveBlock(int x, int y, int z)
    {
        ...
        // SetBlockGenerating, not SetBlock: SetBlock marks the chunk dirty, which reaches into
        // World's shared dirty set and is not safe from a generation worker. A freshly generated
        // chunk is already dirty, so nothing is lost.
        if (block != BlockType.Bedrock && BlockRegistry.IsSolid(block))
            mTargetChunk.SetBlockGenerating(localX, y, localZ, BlockType.Air);
    }
}
