...
/// <summary>
/// The overall shape/layout strategy the retired fixed-area TerrainGen used when building its
/// heightmap. Every one of these presets assumed a world with edges, which no longer exists - the
/// infinite generator ignores the whole enum, and it is no longer selectable when creating a world.
/// Kept only because TerrainGen still references it.
/// </summary>
...
public struct WorldGenSettings
{
    ...
    /// <summary>
    /// Constructs a fully-populated WorldGenSettings for the given theme. Each theme case hardcodes
    /// ocean fluid, coastline, tree density, sky/fog/cloud colors, and light level ranges;
    /// <paramref name="themeIndex"/> maps to <see cref="WorldTheme"/> (cast from a raw int, e.g.
    /// from the UI dropdown or saved world metadata).
    /// </summary>
    public static WorldGenSettings Build(int themeIndex)
    {
