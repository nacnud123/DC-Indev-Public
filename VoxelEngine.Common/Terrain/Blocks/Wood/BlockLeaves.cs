...
public class BlockLeaves : Block
{
    ...
    public override void RandomTick(World world, int x, int y, int z, Random random)
    {
        ...
        if (drop.HasValue)
        {
            ...
            world.AddEntity(new DroppedItemEntity(spawnPos, drop.Value));
        }

        GameContext.Current.ParticleSystem.SpawnBlockBreakParticles(new Vector3(x, y, z), BlockType.Leaves);
