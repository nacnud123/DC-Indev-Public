...
/*Vertex Format
 *Each vertex is 12 floats (48 bytes):
   Offset 	Size 	Data
   ...
   11 	1 float 	Fluid animation: 1 on flowing water/lava, 0 on everything else
 *
 */
...
/// <summary>
/// Pure vertex-emission helpers used while building a chunk's mesh. Each method appends raw floats
/// (see the 11-float vertex layout documented at the top of this file) for one block's worth of
/// geometry — a single face, a full box, a foliage cross, fire, or a torch — directly into the
/// shared vertex buffer passed in. Face culling (deciding *whether* a face should be emitted, based
/// on neighboring block opacity) happens in the caller (<c>Chunk</c>/mesh-rebuild code) before these
/// methods are invoked; everything here assumes the face is already known to be visible and just
/// worries about producing correct geometry, UVs, normals and per-vertex light/shade values.
/// No methods here allocate on the heap in the hot per-face paths (the scalar <see cref="AddVertex(List{float},float,float,float,float,float,float,float,float,float,float,float)"/>
/// overload exists specifically to avoid constructing Vector2/Vector3 per vertex).
/// </summary>
internal static class ChunkMeshBuilder
{
    // Torch model dimensions, all in block-fraction units (divided by 16, i.e. Minecraft's classic
    // 16x16x16 pixel-grid convention for sub-block models).
    ...
    // Wall-mounted torch tilt: sin/cos of a fixed ~22.5 degree lean angle used to rotate the torch
    // model when attached to a wall (vs standing upright on the floor).
    ...
    /// <summary>
    /// Emits one face (2 triangles) of a standard cube block, looking up the correct atlas texture
    /// coordinates for the given face/block via <see cref="BlockRegistry"/>, then delegating to the
    /// explicit-texture overload below. <paramref name="topY"/> optionally overrides the face's top Y
    /// coordinate (used for partial-height blocks like slabs/liquids) instead of the full y+1.
    /// Face culling against neighbors must already have been decided by the caller.
    /// </summary>
    ...
    /// <summary>
    /// Emits one face (2 triangles / 6 vertices) of a standard cube using explicit atlas texture
    /// coordinates (skips the BlockRegistry lookup so callers with pre-resolved coords, or hot loops,
    /// avoid the extra indirection — no heap allocations, vertices written directly into
    /// <paramref name="verts"/>). <paramref name="faceShade"/> is a fixed baked-lighting factor per
    /// face direction (Top brightest at 1.0, Bottom darkest at 0.5, front/back 0.8, left/right 0.7) —
    /// a cheap directional-light approximation independent of the dynamic sky/block light values.
    /// Sky/block light bytes (0..Chunk.MAX_LIGHT) are normalized to [0,1] here for the shader.
    /// </summary>
        ...
        int skyLight, int blockLight, float topY = -1f)
    {
        ...
        // Each case below emits exactly 6 vertices (2 triangles sharing an edge) forming one
        // 1x1 quad on the appropriate cube face, with a fixed outward-facing normal and
        // counter-clockwise winding (as seen from outside the cube, matching the normal direction)
        // so backface culling in the renderer keeps only the outward side visible.
        float nx, ny, nz;
        switch (face)
        {
    ...
    /// <summary>
    /// Emits one face of a fluid cell, where the four top corners may each sit at a different
    /// height. That is what makes flowing water visibly slope: a stream running downhill has a
    /// tilted surface rather than a staircase of flat slabs, and the side faces become trapezoids
    /// that meet the neighbouring cell's surface exactly.
    /// <paramref name="h00"/>..<paramref name="h01"/> are absolute world Y values for the corners
    /// at (x,z), (x+1,z), (x+1,z+1) and (x,z+1) respectively.
    /// </summary>
    internal static void AddFluidFace(List<float> verts, float x, float y, float z, Chunk.Face face,
        BlockType block, int skyLight, int blockLight, float h00, float h10, float h11, float h01,
        bool animate)
    {
        // Per-vertex rather than a uniform: one draw call covers a whole chunk, so a still pond
        // and the stream feeding it are in the same batch and must be told apart per vertex.
        float anim = animate ? 1f : 0f;
        ...
        float sky = skyLight / (float)Chunk.MAX_LIGHT;
        float light = blockLight / (float)Chunk.MAX_LIGHT;

        var texCoords = face == Chunk.Face.Top
            ? BlockRegistry.GetTopTexture(block)
            : face == Chunk.Face.Bottom
                ? BlockRegistry.GetBottomTexture(block)
                : BlockRegistry.GetSideTexture(block);
        ...
        // On a side face the texture is cropped to how much of the block the fluid actually fills,
        // per corner - so a sloping surface doesn't stretch the texture along the slope.
        float VAt(float h) => v0 + (v1 - v0) * (h - y);

        switch (face)
        {
            case Chunk.Face.Front: // +Z, top edge spans corners (x,z+1) and (x+1,z+1)
                AddVertex(verts, x, y, z + 1, sky, light, faceShade, 0, 0, 1, u0, v0, anim);
                AddVertex(verts, x + 1, h11, z + 1, sky, light, faceShade, 0, 0, 1, u1, VAt(h11), anim);
                AddVertex(verts, x, h01, z + 1, sky, light, faceShade, 0, 0, 1, u0, VAt(h01), anim);
                AddVertex(verts, x, y, z + 1, sky, light, faceShade, 0, 0, 1, u0, v0, anim);
                AddVertex(verts, x + 1, y, z + 1, sky, light, faceShade, 0, 0, 1, u1, v0, anim);
                AddVertex(verts, x + 1, h11, z + 1, sky, light, faceShade, 0, 0, 1, u1, VAt(h11), anim);
                break;

            case Chunk.Face.Back: // -Z, top edge spans corners (x,z) and (x+1,z)
                AddVertex(verts, x, y, z, sky, light, faceShade, 0, 0, -1, u1, v0, anim);
                AddVertex(verts, x, h00, z, sky, light, faceShade, 0, 0, -1, u1, VAt(h00), anim);
                AddVertex(verts, x + 1, h10, z, sky, light, faceShade, 0, 0, -1, u0, VAt(h10), anim);
                AddVertex(verts, x, y, z, sky, light, faceShade, 0, 0, -1, u1, v0, anim);
                AddVertex(verts, x + 1, h10, z, sky, light, faceShade, 0, 0, -1, u0, VAt(h10), anim);
                AddVertex(verts, x + 1, y, z, sky, light, faceShade, 0, 0, -1, u0, v0, anim);
                break;

            case Chunk.Face.Top:
                AddVertex(verts, x, h00, z, sky, light, faceShade, 0, 1, 0, u0, v0, anim);
                AddVertex(verts, x + 1, h11, z + 1, sky, light, faceShade, 0, 1, 0, u1, v1, anim);
                AddVertex(verts, x + 1, h10, z, sky, light, faceShade, 0, 1, 0, u1, v0, anim);
                AddVertex(verts, x, h00, z, sky, light, faceShade, 0, 1, 0, u0, v0, anim);
                AddVertex(verts, x, h01, z + 1, sky, light, faceShade, 0, 1, 0, u0, v1, anim);
                AddVertex(verts, x + 1, h11, z + 1, sky, light, faceShade, 0, 1, 0, u1, v1, anim);
                break;

            case Chunk.Face.Bottom: // flat - the underside of a fluid is always the cell floor
                AddVertex(verts, x, y, z, sky, light, faceShade, 0, -1, 0, u0, v1, anim);
                AddVertex(verts, x + 1, y, z, sky, light, faceShade, 0, -1, 0, u1, v1, anim);
                AddVertex(verts, x + 1, y, z + 1, sky, light, faceShade, 0, -1, 0, u1, v0, anim);
                AddVertex(verts, x, y, z, sky, light, faceShade, 0, -1, 0, u0, v1, anim);
                AddVertex(verts, x + 1, y, z + 1, sky, light, faceShade, 0, -1, 0, u1, v0, anim);
                AddVertex(verts, x, y, z + 1, sky, light, faceShade, 0, -1, 0, u0, v0, anim);
                break;

            case Chunk.Face.Right: // +X, top edge spans corners (x+1,z) and (x+1,z+1)
                AddVertex(verts, x + 1, y, z, sky, light, faceShade, 1, 0, 0, u1, v0, anim);
                AddVertex(verts, x + 1, h10, z, sky, light, faceShade, 1, 0, 0, u1, VAt(h10), anim);
                AddVertex(verts, x + 1, h11, z + 1, sky, light, faceShade, 1, 0, 0, u0, VAt(h11), anim);
                AddVertex(verts, x + 1, y, z, sky, light, faceShade, 1, 0, 0, u1, v0, anim);
                AddVertex(verts, x + 1, h11, z + 1, sky, light, faceShade, 1, 0, 0, u0, VAt(h11), anim);
                AddVertex(verts, x + 1, y, z + 1, sky, light, faceShade, 1, 0, 0, u0, v0, anim);
                break;

            case Chunk.Face.Left: // -X, top edge spans corners (x,z+1) and (x,z)
                AddVertex(verts, x, y, z + 1, sky, light, faceShade, -1, 0, 0, u1, v0, anim);
                AddVertex(verts, x, h01, z + 1, sky, light, faceShade, -1, 0, 0, u1, VAt(h01), anim);
                AddVertex(verts, x, h00, z, sky, light, faceShade, -1, 0, 0, u0, VAt(h00), anim);
                AddVertex(verts, x, y, z + 1, sky, light, faceShade, -1, 0, 0, u1, v0, anim);
                AddVertex(verts, x, h00, z, sky, light, faceShade, -1, 0, 0, u0, VAt(h00), anim);
                AddVertex(verts, x, y, z, sky, light, faceShade, -1, 0, 0, u0, v0, anim);
                break;
        }
    }

    /// <summary>
    /// Builds Minecraft-style fire geometry as a cluster of double-sided quads. When the fire has a
    /// solid block base (<paramref name="hasSolidBase"/> true, e.g. fire sitting on a normal block),
    /// it draws the classic 8-quad "campfire" pattern of diagonal panels leaning slightly inward from
    /// the block's edges. When fire has no solid base (e.g. it's clinging to the side of a
    /// flammable block after spreading, sitting on grass/etc. that burned away), it instead draws
    /// leaning wall-panels only on the sides that are adjacent to a flammable neighbor
    /// (<paramref name="flammableNegX"/> etc.), plus an X-shaped cap quad pair on top when the block
    /// above is flammable — mirroring how vanilla fire clings to whatever fuel is actually present.
    /// </summary>
    ...
    /// <summary>
    /// Emits a fire panel as two triangle pairs — one winding for the front face, one with reversed
    /// winding for the back face — so the flat quad is visible from both sides (fire has no back-face
    /// culling benefit since it's a thin plane, unlike a solid cube face).
    /// </summary>
    ...
    /// <summary>
    /// Builds foliage-style geometry (grass, flowers, saplings, etc.) as two intersecting diagonal
    /// quads forming an X when viewed from above, each rendered double-sided (front+back triangle
    /// sets) so the sprite is visible from any horizontal angle. Unlike cube faces, cross blocks are
    /// never culled against neighbors — they always render fully regardless of what's adjacent. No
    /// heap allocations (uses the scalar AddVertex overload throughout).
    /// </summary>
    ...
    /// <summary>
    /// Builds a torch as a thin vertical box (pole) sized/offset by the TORCH_* constants so it sits
    /// centered and short on top of its block. When <paramref name="facing"/> is -1 the torch stands
    /// upright on the floor as-is; when facing is 0-3 (one of the 4 cardinal wall directions) every
    /// corner is rotated via <see cref="RotateWallTorch"/> to tilt the pole outward from a wall
    /// mount, matching vanilla wall-torch placement. The 6 box faces are emitted individually with
    /// their own per-face shade constant (same top/bottom/side shading scheme as full cube faces).
    /// </summary>
        ...
        int blockLight, int facing = -1)
    {
        ...
        // The 8 corners of the torch's thin bounding box, in a fixed index order so the quad
        // emission calls below (corners[i]) can pick out the 4 corners of each face by index.
        Vector3[] corners =
        [
        ...
        // The top face is drawn slightly recessed below the pole's true top (y1) to mimic the
        // torch's flame/tip visually sitting inside the pole rather than flush with its top edge.
    ...
    /// <summary>
    /// Rotates a torch-model point about a pivot to tilt it outward from a wall for the given
    /// cardinal <paramref name="facing"/> (0-3). Translates the point into pivot-local space, applies
    /// a fixed-angle 2D rotation (using the precomputed TILT_SIN/TILT_COS) in the plane perpendicular
    /// to the wall, then re-adds the pivot and a constant <see cref="WALL_SHIFT"/> push away from the
    /// wall face so the tilted pole doesn't clip into the mounting block. Facings outside 0-3 return
    /// the point unchanged (defensive default).
    /// </summary>
    ...
    // fluidAnim defaults to 0, so every non-fluid caller writes "don't animate" without knowing
    // this channel exists.
    private static void AddVertex(List<float> verts, Vector3 pos, float skyLightNorm, float blockLightNorm,
        float faceShade, Vector3 normal, Vector2 uv, float fluidAnim = 0f)
    {
        verts.Add(pos.X);
        verts.Add(pos.Y);
        verts.Add(pos.Z);
