...
public class FurnaceData : IBlockEntity
{
    ...
    public void DropContents(World world)
    {
        ...
        if (InputSlot.HasValue)
            world.AddEntity(new DroppedItemEntity(center, InputSlot.Value));

        if (FuelSlot.HasValue)
            world.AddEntity(new DroppedItemEntity(center, FuelSlot.Value));

        if (OutputSlot.HasValue)
            world.AddEntity(new DroppedItemEntity(center, OutputSlot.Value));
